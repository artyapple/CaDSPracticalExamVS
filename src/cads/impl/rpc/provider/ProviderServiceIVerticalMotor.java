...
import cads.impl.hal.IVerticalMotor;

public class ProviderServiceIVerticalMotor implements Runnable {
	...
	public void run() {
		try {
			...
			service = fac.getInstance(IVerticalMotor.class);
		...
		while (true) {
			try {
				...
				switch (function.getName()) {
					case "moveToPercent": 
service.moveToPercent(
message.intValueFor("percent"));
break;
					default:
						System.out.println("ProviderServiceIVerticalMotor: " + "can not handle " + function.getName() + " function");
						break;
				}
